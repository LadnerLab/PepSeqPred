"""bkp_eval_pipeline.py

Utilities for BKP FFNN evaluation orchestration:
...
def run_prepare(args: argparse.Namespace) -> None:
    """Builds reduced evaluation metadata and FASTA from BKP source files."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="BKP evaluation helper for subset preparation and peptide-level comparison."
    )
    ...
    prepare.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory containing BKP dataset source files."
    )
    prepare.add_argument(
        "--output-dir",
        type=Path,
    ...
    prepare.add_argument(
        "--meta-file",
        type=str,
        default="BKP1_meta_wClusterInfo_StartStopAdded_70ClustAlignAdded.tsv",
        help="Metadata TSV filename under --data-dir."
    )
    prepare.add_argument(
        "--proteins-fasta",
        type=str,
        default="BKP1_usedTargets.fasta",
        help="Protein FASTA filename under --data-dir."
    )
    prepare.add_argument(
        "--reactive-file",
        type=str,
        default="BKP_reactive_Z20N4.tsv",
        help="Reactive peptide list filename under --data-dir."
    )
    prepare.add_argument(
        "--nonreactive-file",
        type=str,
        default="BKP_non-reactive_Z10N0.tsv",
