...
Logging setup utilities for PepSeqPred CLIs.

Provides a configurable logger factory that can emit either standard log
messages or JSON-formatted records to both console and rotating log files.
"""
...
def setup_logger(log_dir: Optional[Path] = None, log_level: str = "INFO", json_lines: bool = False, json_indent: Optional[int] = None, name: str = Path(__file__).stem) -> logging.Logger:
    ...
    if log_dir is not None:
        ...
        log_path = log_dir / \
            f"{name}_{datetime.now().strftime('%Y-%m-%d_T%H_%M_%S')}.log"
    ...
    formatter = JSONFormatter() if json_lines else logging.Formatter(
        "[%(asctime)s] %(levelname)s %(message)s")
    stream_formatter = JSONFormatter() if json_lines else logging.Formatter(
