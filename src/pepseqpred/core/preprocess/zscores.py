...
Z-score preprocessing utilities for PepSeqPred.

Provides helpers to apply epitope thresholding on subject z-scores and to merge
the resulting targets with PV1 metadata tables.
"""
...
def merge_zscores_metadata(z_df: pd.DataFrame,
                           ...
                           target_cols: List[str] = [
                               "Def epitope", "Uncertain", "Not epitope"],
...
def apply_z_threshold(z_df: pd.DataFrame,
    ...
    if not_epitope_max_subjects is not None:
        def_not = (nonreactive_counts >= not_epitope_max_subjects) & (
            reactive_counts == 0)
    else:
